...
class CounterFlowPOSScreen(QWidget):
    ...
    def _counterflow_build_right_panel(self) -> QWidget:
        ...
        counterflow_find_btn = QPushButton("Find")
        counterflow_find_btn.setFixedWidth(52)
        counterflow_find_btn.setFixedHeight(40)
        counterflow_find_btn.setObjectName("counterflowOutlineBtn")
        counterflow_find_btn.clicked.connect(self._counterflow_on_mobile_entered)

        counterflow_mobile_row.addWidget(self._counterflow_mobile_input)
        counterflow_mobile_row.addWidget(counterflow_find_btn)
    ...
    def _counterflow_update_pay_buttons(self):
        ...
        """
        counterflow_ready = self._counterflow_lookup_done
    ...
    def counterflow_refresh_theme(self):
        ...
        # Total card — inverted card (uses text_primary as bg so it
        # stands out; in dark mode text_primary is near-white → correct)
        self._counterflow_total_card.setStyleSheet(f"""
            QFrame {{
                background: {thm['text_primary']};
                border-radius: 12px;
